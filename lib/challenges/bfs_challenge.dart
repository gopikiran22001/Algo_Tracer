...
class BFSChallenge extends StatefulWidget {
  final AlgorithmModel algorithm;

  const BFSChallenge({super.key, required this.algorithm});

  @override
  State<BFSChallenge> createState() => _BFSChallengeState();
}

class _BFSChallengeState extends State<BFSChallenge> {
  ...
  final List<String> _correctOrder = ['A', 'B', 'C', 'D', 'E', 'F'];
  ...
  void _selectNode(String node) {
    ...
    if (node == _correctOrder[_currentIndex]) {
      setState(() {
        _visited.add(node);
        _currentIndex++;
        _hint = 'Correct! BFS visits level by level';
      });
      ...
      setState(() {
        ...
        _hint = 'Wrong! BFS visits all nodes at current level first';
      });
    }
  }
